...
import (
	...
	"example.com/inkkeep/internal/util/filedump"
	"example.com/inkkeep/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "InkKeep TAC", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "inkkeep"},
	}
